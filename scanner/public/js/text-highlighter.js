    ...
    init: function (siteName, flag) {
        ...
        const wrapElements = (el) => {
            ...
            if (el?.previousElementSibling && !el.previousElementSibling.parentNode.classList.contains("dom-texthighlight")) {
                ...
                if (prevSibling) {
                    ...
                    wrapper.classList.add('d-inline-flex', 'align-items-center', 'gap-1', 'dom-texthighlight');
            ...
            tables.forEach(function (table) {
                ...
                rows.forEach(row => {
                    ...
                    highlightIndices.forEach(index => {
                        ...
                        else if (actualElement.parentNode.parentNode.getAttribute('data-highlight-target')) {
                            content = actualElement.parentNode.parentNode.getAttribute('data-highlight-target').toLowerCase();
                        }
        ...
        ; function doMouseEnter() {
            ...
            if (!(content.startsWith("Create: ") || content.startsWith("Contract Creation"))) {
                try {
                    if (!TextHighlight.contentMap[content]) return;
