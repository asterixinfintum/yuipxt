function blockChainSearchInit({ id, isMainPanel = true, classResultWrapper, groupIncludes = [], attributes, is_boosted_enabled = false, history_orientation = 'vertical', enable_search_history = true }) {
    ...
    const SEARCH_HISTORY_KEY = 'etherSearchHistory';
    const MAX_HISTORY_ITEMS = 5;

    function getSearchHistory() {
        try {
            if (enable_search_history === false) return [];
    ...
    function saveSearchHistory(item) {
        try {
            ...
            // Remove duplicate if exists (based on address or title)
            history = history.filter(h => h.address !== item.address && h.title !== item.title);
            
            // Add new item at the beginning (encoding is applied on render via htmlEncode)
            history.unshift({
                title: item.title,
    ...
    function removeSearchHistoryItem(address) {
        try {
            if (enable_search_history === false) return;
    ...
    function renderSearchHistoryHTML() {
        ...
        if (history_orientation === 'vertical') {
            // Vertical layout (list view)
            const historyItems = history.map((item, index) => `
                <li class="nav-item nav-link mb-2 rounded text-start" style="cursor: pointer;" data-address="${htmlEncode(item.address)}" role="option" tabindex="-1" aria-selected="false" aria-setsize="${history.length}" aria-posinset="${index}">
                    <span class="d-none">${htmlEncode(item.address)}</span>
                    <a href="${htmlEncode(item.link)}" class="nav-link p-2 w-100">
                        <h6 class="d-flex align-items-center fw-normal mb-0 gap-1.5">
                            <i class="far fa-clock text-secondary"></i>
                            <div class="text-truncate">${htmlEncode(item.title)}</div>
                        </h6>
                    </a>
                </li>
            `).join('');
            ...
            return `
                <ul tabindex="0" role="listbox" class="search-panel-history search-panel-tags-container mb-n0.5 nav-pills nav-pills-flush nav-list">
                    <div class="search-category-history text-cap fw-medium px-2 pt-3 pb-1 mb-0 d-flex justify-content-between align-items-center">
                        <span>Recent Searches</span>
                        <button type="button" class="btn btn-link btn-sm p-0 text-muted clear-history-btn" style="text-decoration: none;">
                            Clear All
                            <i class="far fa-trash fa-sm ms-0.5"></i>
                        </button>
                    </div>
                    ${historyItems}
                </ul>
            `;
        } else if (history_orientation === 'horizontal') {
            // Horizontal layout (chip/badge view)
            const historyChips = history.map((item) => `
                <a href="${htmlEncode(item.link)}" class="badge bg-white hover:bg-secondary border border-dark dark:border-white border-opacity-15 text-dark text-nowrap fw-normal transition-all py-1.5 px-2 fs-sm d-flex align-items-center gap-0.5" style="cursor: pointer;" data-address="${htmlEncode(item.address)}">
                    ${item.img ? `<img src="${htmlEncode(item.img)}" width="12" class="rounded-circle" alt="${htmlEncode(item.title)}">` : ''}
                    ${htmlEncode(item.title)}
                </a>
            `).join('');
            
            return `
                <ul class="search-panel-history search-panel-tags-container mb-0.5">
                    <div class="search-category-history text-cap fw-medium px-2 pt-3 pb-1 mb-0 d-flex justify-content-between align-items-center">
                        <span>Recent Searches</span>
                        <button type="button" class="btn btn-link btn-sm p-0 text-muted clear-history-btn" style="text-decoration: none;">
                            Clear All
                    ...
                    <div class="overflow-x-auto scrollbar-custom d-flex gap-1 px-2 py-1">
                        ${historyChips}
                    </div>
                </ul>
            `;
    ...
    function showSearchHistoryPanel() {
        ...
        const historyPanel = mainContainer.querySelector('.search-panel-history');
        if (historyPanel) {
    ...
    function updateSearchHistoryPanel() {
        ...
        const existingPanel = mainContainer.querySelector('.search-panel-history');
        const newHTML = renderSearchHistoryHTML();
        
        if (existingPanel && existingPanel.parentNode) {
            // Update existing panel (only if it still has a parent node)
            if (newHTML) {
                try {
                    existingPanel.outerHTML = newHTML;
                    attachClearHistoryEvent();
                    attachPanelLinkEvents();
                } catch (e) {
                    // Panel may have been removed from DOM, ignore error
                }
            } else {
                existingPanel.remove();
            }
        } else if (newHTML) {
            // Double-check no panel exists before inserting (prevent duplicates)
            if (mainContainer.querySelector('.search-panel-history')) {
                return;
            }
            
            // Create new panel if history exists but panel doesn't
            // Insert before trending panel (to keep history above trending), or before search header
            const trendingPanel = mainContainer.querySelector('.search-panel-trending');
            const searchHeader = mainContainer.querySelector('.search-panel-header');
            
            if (trendingPanel && trendingPanel.parentNode) {
                trendingPanel.insertAdjacentHTML('beforebegin', newHTML);
                attachClearHistoryEvent();
                attachPanelLinkEvents();
            } else if (searchHeader && searchHeader.parentNode) {
                searchHeader.insertAdjacentHTML('beforebegin', newHTML);
                attachClearHistoryEvent();
                attachPanelLinkEvents();
            }
        }
    ...
    function attachClearHistoryEvent() {
        if (enable_search_history === false) return;

        const clearBtn = mainContainer.querySelector('.clear-history-btn');
        if (clearBtn) {
            // Use mousedown instead of click to fire before autocomplete's document click handler
    ...
    function attachPanelLinkEvents() {
        if (enable_search_history === false) return;

        // Attach mousedown events to all links in history, boosted, and trending panels
        const panels = ['.search-panel-history', '.search-panel-boosted', '.search-panel-trending'];
        
        panels.forEach(panelSelector => {
            const panel = mainContainer.querySelector(panelSelector);
            if (panel) {
                const links = panel.querySelectorAll('a[href]');
                links.forEach(link => {
                    // Remove existing listener to prevent duplicates
                    link.removeEventListener('mousedown', handlePanelLinkClick);
                    // Add mousedown listener
                    link.addEventListener('mousedown', handlePanelLinkClick);
                });
            }
        });
    }
    ...
    // Boosted Section Functions
    // ==========================================

    function fetchBoostedData() {
        if (enable_search_history === false) return;

        // Mock API call - returns a promise to simulate async behavior
                ...
                resolve([
                    {
                        ...
                        price: '$3,438.80',
                        boost: '3.21k',
                        img: '../assets/svg/brands/weth.svg',
                        link: '/token/0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
                    },
                    {
                        title: 'Tether USD',
                        symbol: 'USDT',
                        price: '$1.00',
                        boost: '2.85k',
                        img: '../assets/svg/brands/usdt.svg',
                        link: '/token/0xdac17f958d2ee523a2206206994597c13d831ec7'
                    },
                    {
                        title: 'USD Coin',
                        symbol: 'USDC',
                        price: '$1.00',
                        boost: '1.92k',
                        img: '../assets/svg/brands/usdc.svg',
                        link: '/token/0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'
                    }
                ]);
            }, 100);
        });
    ...
    function renderBoostedHTML(boostedData) {
        if (enable_search_history === false) return '';
        ...
        const boostedItems = boostedData.map((item) => `
            <div class="col-md-4">
                <a href="${item.link}">
                    <div class="card border-2 position-relative" style="--bs-border-color: transparent; background: rgba(183, 110, 0,0.05); background-clip: padding-box;">
                        <div style="position: absolute; inset: -2px; border-radius: inherit; padding: 2px; background: linear-gradient(to top, #EF9721, #FED595); -webkit-mask: linear-gradient(#fff, #fff) content-box, linear-gradient(#fff, #fff); -webkit-mask-composite: xor; mask-composite: exclude; pointer-events: none; z-index: -1;"></div>
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <img width="24" class="rounded-1" src="${item.img}" alt="${item.symbol}">
                                <div class="d-flex align-items-center gap-1 fs-sm" style="color: #B76E00;">
                                    <i class="far fa-rocket"></i>
                                    ${item.boost}
                                </div>
                            </div>
                            <div style="color: var(--bs-body-color);">${item.title}</div>
                            <div class="fs-sm text-muted">${item.price}</div>
                        </div>
                    </div>
                </a>
            </div>
        ...
        return `
            <ul class="search-panel-boosted search-panel-tags-container mb-0.5">
                <div class="px-2 pt-3 pb-1">
                    <div class="text-cap fw-medium pb-2 mb-0">Top Boosted</div>
                    <div class="row g-2">
                        ${boostedItems}
                    </div>
                </div>
    ...
    function showBoostedPanel() {
        if (enable_search_history === false) return;

        const boostedPanel = mainContainer.querySelector('.search-panel-boosted');
        if (boostedPanel) {
            boostedPanel.classList.remove('d-none');
        }
    }

    function hideBoostedPanel() {
        if (enable_search_history === false) return;

        const boostedPanel = mainContainer.querySelector('.search-panel-boosted');
        if (boostedPanel) {
            boostedPanel.classList.add('d-none');
        }
    }

    async function updateBoostedPanel() {
        if (enable_search_history === false) return;

        // Fetch boosted data from mock API
        const boostedData = await fetchBoostedData();
        const newHTML = renderBoostedHTML(boostedData);
        
        // Re-query after async fetch to get fresh reference
        const existingPanel = mainContainer.querySelector('.search-panel-boosted');
        ...
        if (existingPanel && existingPanel.parentNode) {
            ...
            // Double-check no panel exists before inserting (prevent duplicates)
            if (mainContainer.querySelector('.search-panel-boosted')) {
                return;
            }
            
            // Insert after history panel, or before trending/search header
            const historyPanel = mainContainer.querySelector('.search-panel-history');
            const trendingPanel = mainContainer.querySelector('.search-panel-trending');
            const searchHeader = mainContainer.querySelector('.search-panel-header');
            
            if (historyPanel && historyPanel.parentNode) {
                historyPanel.insertAdjacentHTML('afterend', newHTML);
                attachPanelLinkEvents();
            } else if (trendingPanel && trendingPanel.parentNode) {
                trendingPanel.insertAdjacentHTML('beforebegin', newHTML);
                attachPanelLinkEvents();
            } else if (searchHeader && searchHeader.parentNode) {
                searchHeader.insertAdjacentHTML('beforebegin', newHTML);
    ...
    function renderTrendingHTML(trendingData) {
        if (enable_search_history === false) return '';
        ...
        const trendingItems = trendingData.map((item) => `
            <li class="nav-item nav-link mb-2 rounded text-start" style="cursor: pointer;" role="option" data-address="${item.address}">
                <a href="${item.link}" class="nav-link p-2 d-flex align-items-start w-100">
                    <div class="me-2">
                        <img src="${item.img}" width="22" class="rounded-circle" alt="Token">
                    </div>
                    <div class="flex-fill text-truncate">
                        <h6 class="d-flex align-items-center fw-normal mb-0 gap-2">
                            <div class="text-truncate">${item.title}</div>
                            ${item.rate ? `<span class="badge bg-secondary bg-opacity-15 text-dark fw-medium p-1">${item.rate}</span>` : ''}
                            ${item.is_checked ? '<i class="fa fa-badge-check text-info ms-auto"></i>' : ''}
                        </h6>
                        <span class="text-muted text-truncate small">${item.address}</span>
                    </div>
                </a>
            </li>
        `).join('');
        ...
        return `
            <ul tabindex="0" role="listbox" class="search-panel-trending search-panel-tags-container mb-n0.5 nav-pills nav-pills-flush nav-list">
                <div class="text-cap fw-medium px-2 pt-3 pb-1 mb-0">🔥 Trending Search</div>
                ${trendingItems}
            </ul>
        `;
    }
    ...
    function showTrendingPanel() {
        ...
        const trendingPanel = mainContainer.querySelector('.search-panel-trending');
        if (trendingPanel) {
            trendingPanel.classList.remove('d-none');
        }
    }

    function hideTrendingPanel() {
        if (enable_search_history === false) return;

        const trendingPanel = mainContainer.querySelector('.search-panel-trending');
        if (trendingPanel) {
            trendingPanel.classList.add('d-none');
        }
    }

    async function updateTrendingPanel() {
        if (enable_search_history === false) return;

        // Fetch trending data from JSON file
        const trendingData = await fetch('../assets/js/custom/search-trending.json')
            .then(response => response.json())
            .catch(() => []);
        
        const newHTML = renderTrendingHTML(trendingData);
        
        // Re-query after async fetch to get fresh reference
        const existingPanel = mainContainer.querySelector('.search-panel-trending');
        ...
        if (existingPanel && existingPanel.parentNode) {
            ...
            // Double-check no panel exists before inserting (prevent duplicates)
            if (mainContainer.querySelector('.search-panel-trending')) {
                return;
            }
            
            // Insert after boosted panel, or after history, or before search header
            const boostedPanel = mainContainer.querySelector('.search-panel-boosted');
            const historyPanel = mainContainer.querySelector('.search-panel-history');
            const searchHeader = mainContainer.querySelector('.search-panel-header');
            
            if (boostedPanel && boostedPanel.parentNode) {
                boostedPanel.insertAdjacentHTML('afterend', newHTML);
                attachPanelLinkEvents();
            } else if (historyPanel && historyPanel.parentNode) {
                historyPanel.insertAdjacentHTML('afterend', newHTML);
                attachPanelLinkEvents();
            } else if (searchHeader && searchHeader.parentNode) {
                searchHeader.insertAdjacentHTML('beforebegin', newHTML);
    ...
    function redirectBasedOnGroupId({ searchkey = '', triggerNewTab = false, returnLink = false }) {
        ...
        let redirect_link = (() => {
            ...
            return '/search?f=0&q=' + encodeURIComponent(searchkey);
        })();
    ...
    searchInput.addEventListener("keydown", (event) => {
        if ((event.code === 'Enter' || event.keyCode === 13) && searchInput == document.activeElement && id !== 'dropdownSearchFilterInputAdvanceSearch') {
            // Save direct search to history before redirecting
            if (event.target.value.trim()) {
                saveSearchHistory({
                    title: event.target.value,
                    address: event.target.value,
                    link: redirectBasedOnGroupId({ searchkey: event.target.value, returnLink: true }),
                    img: ''
                });
            }
    ...
    // Handle showing/hiding search history, boosted and trending based on input value
    searchInput.addEventListener("input", (event) => {
        const inputValue = event.target.value.trim();
        
        if (inputValue.length >= 1) {
            // Hide search history, boosted and trending when user starts typing
            hideSearchHistoryPanel();
            if (is_boosted_enabled) hideBoostedPanel();
            hideTrendingPanel();
        } else {
            // Show search history, boosted and trending when input is empty
            showSearchHistoryPanel();
            if (is_boosted_enabled) showBoostedPanel();
            showTrendingPanel();
        }
    });
    ...
    searchInput.addEventListener("focus", () => {
        ...
        // Show search history, boosted and trending if input is empty
        if (searchInput.value.trim() === '') {
            showSearchHistoryPanel();
            updateSearchHistoryPanel();
            if (is_boosted_enabled) {
                showBoostedPanel();
                updateBoostedPanel();
            }
            showTrendingPanel();
    ...
    if (clearIcon) {
        clearIcon.addEventListener("click", () => {
            ...
            // Show search history, boosted and trending when input is cleared
            showSearchHistoryPanel();
            updateSearchHistoryPanel();
            if (is_boosted_enabled) {
                showBoostedPanel();
    ...
    let autocomplete = new Autocomplete(id, {
        ...
        onResults: ({ currentValue, matches, template, classGroup }) => {
            ...
            if (isMainPanel || isAdvancedFilterSearch) {
                mainContainer.querySelector('.search-panel-header').innerHTML = `<div class="search-panel-tags-container d-flex overflow-x-auto scrollbar-custom gap-1 px-4 py-3">${[...new Set(matches
            ...
            return matches === 0
                ...
                : matches.map((el, index, array) => {
                    ...
                    if (searchResultLink.indexOf('tokenflowchart') > -1) {
                        ...
                        onSearchResultClick = `onclick="reloadTokenFlow('${htmlEncode(el[attributes.address].replace(/'/g, "\\'"))}', 90);"`
                    }

                    return `${group}
                    <li class="nav-item nav-link ${(isMainPanel || isAdvancedFilterSearch) ? 'mb-2' : 'my-2'} rounded" style="cursor: pointer;">
                        <span class="d-none">${htmlEncode(el[attributes.address] || el[attributes.name])}</span>
                        <a href="${htmlEncode(searchResultLink)}" ${onSearchResultClick} class="nav-link p-2 d-flex ${el[attributes.address] ? 'align-items-start' : 'align-items-center'} w-100">
                            <div class="me-2">
                                ${el[attributes.groupid] === '3' || el[attributes.groupid] === '2'
                            ? `<img src="${htmlEncode(img_url)}" width="22" class="rounded-circle" alt="${htmlEncode(altText)}">` : ''}
                            ...
                            <div class="flex-fill text-truncate">
                                <h6 class="d-flex align-items-center fw-normal mb-0">
                                    <div class="text-truncate me-2">${htmlEncode(el[attributes.name])}</div>
                                    ${el[attributes.rate] ? `<span class="badge text-dark fw-medium p-1">${htmlEncode(el[attributes.rate])}</span>` : ''}
                                ...
                                ${el[attributes.address] && el[attributes.name] !== '' ? `<span class="text-muted text-truncate small">${htmlEncode(el[attributes.address])}</span>` : `<span class="text-muted text-truncate">${htmlEncode(el[attributes.address])}</span>`}
                                ${el[attributes.website] ? `<div class="text-muted text-truncate small">${htmlEncode(el[attributes.website])}</div>` : ''}
                            </div>
                        </a>
                    </li>`;
            }
        ...
        onSubmit: ({ index, object }) => {
            ...
            // Save to search history before redirecting
            if (selectedElem && object) {
                saveSearchHistory({
                    title: object[attributes.name] || object[attributes.address] || searchInput.value,
                    address: object[attributes.address] || searchInput.value,
                    link: selectedElem.childNodes[3].href,
                    img: object[attributes.img] || ''
                });
            } else if (selectedElem) {
                // Fallback for items without object data
                const displayText = selectedElem.querySelector('.text-truncate')?.textContent || searchInput.value;
                const address = selectedElem.dataset.address || searchInput.value;
                const imgElem = selectedElem.querySelector('img');
                saveSearchHistory({
                    title: displayText,
                    address: address,
                    link: selectedElem.childNodes[3].href,
        ...
        onRender: async ({ results }) => {
            if (isMainPanel || isAdvancedFilterSearch) {
                if (isMainPanel) {
                    // Generate dynamic search history HTML
                    const searchHistoryHTML = renderSearchHistoryHTML();
                    
                    // Fetch boosted data from mock API (only if enabled)
                    let boostedHTML = '';
                    if (is_boosted_enabled) {
                        const boostedData = await fetchBoostedData();
                        boostedHTML = renderBoostedHTML(boostedData);
                    }
                    ...
                    const trendingData = await fetch('../assets/js/custom/search-trending.json')
                        .then(response => response.json())
                        .catch(() => []);
                    const trendingHTML = renderTrendingHTML(trendingData);
                    
                    results.insertAdjacentHTML('beforebegin', getSeachBoxAd() + `
                        ${searchHistoryHTML}
                        ${boostedHTML}
                        ${trendingHTML}
                        <div class="search-panel-header"></div>`);
                }
                // Attach clear history button event and panel link events
                attachClearHistoryEvent();
                attachPanelLinkEvents();
