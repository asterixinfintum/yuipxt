...
function onERC721DocReady() {
    ...
    if (obj.value == "" || obj.value == strAge) {
        ...
        //show UTC with timezone on column header
        if (offsetHoursFinal !== undefined) {
            $("#lnkErc721AgeDateTime").text(strDateTimeLocal);
        }
    }
...
function switchMethodColumn(el, text1, text2, isStart) {
    if (el) {
        var res = getCookie(strSiteName + "_switch_function_name");

        if ((isStart == false && res == "method") || (isStart == true && res == "action")) {
            $(el).html(text1)
            $(el).attr("data-bs-toggle", "tooltip")
            $(el).attr("data-bs-trigger", "hover")
            $(el).attr("data-bs-title", "Click to show curated Action where applicable")
            $(".td_functionNameOri").show()
            $(".td_functionNameCustom").hide()
            $(".method-tooltip").show()
            $(".action-tooltip").hide()
            writeCookie(strSiteName + "_switch_function_name", "action")
        } else {
            $(el).html(text2)
            $(el).attr("data-bs-toggle", "tooltip")
            $(el).attr("data-bs-trigger", "hover")
            $(el).attr("data-bs-title", "Click to show executed Function")
            $(".td_functionNameOri").hide()
            $(".td_functionNameCustom").show()
            $(".method-tooltip").hide()
            $(".action-tooltip").show()
            writeCookie(strSiteName + "_switch_function_name", "method")
        }

        // Get the existing tooltip instance
        let tooltipInstance = bootstrap.Tooltip.getInstance(el);
        if (tooltipInstance) {
            // Update the content of the tooltip while it's still shown
            tooltipInstance.setContent({
                '.tooltip-inner': $(el).attr("data-bs-title")
            });
            tooltipInstance.update(); // Refresh the tooltip position and size
        } else {
            // Create a new tooltip instance if not already initialized
            new bootstrap.Tooltip(el, {
                title: $(el).attr("data-bs-title"),
                trigger: 'hover'
            });
        }
    }
    TextHighlight.init();
    $("#headerMethod").show()
    $("#headerMethod_internaltx").show()
    $("#headerMethod_AA").show()
    $("#headerMethod_label").show()
...
function handleTxPreviewOnClick(e) {
    ...
    if (txPreviewPopoverGlobal !== null) {
        if (
            ...
            // if the same element is clicked again, need to hide the popover and do nothing
            let currrentModel = bootstrap.Popover.getOrCreateInstance(targetedClass)
            if (currrentModel && currrentModel._isShown()) {
                currrentModel.hide();
                return;
            }
        }
...
function generateCopyIcon(el, tooltipText) {
    ...
    var oriTooltip = $(el).attr('data-bs-original-title') || $(el).attr('data-original-title') || $(el).attr('title') || "Copy";
    ...
    $(el).attr('data-bs-original-title', tooltipText);
    ...
    setTimeout(() => {
        ...
        $(el).attr('data-bs-original-title', oriTooltip);
        $(el).attr('title', oriTooltip);
    }, 1000)
}
    ...
    $('.theme-btn').click(function () {
        ...
        $("iframe").each(function () {
            ...
            if (src.prevObject.prevObject[0].id === "token_analytics_pageiframe") {
                ...
                if (cThemeMode === 'light' || themeMode === 'light') {
                    ...
                    htmlSrc.attr('data-bs-theme', getCookie('site_auto_theme'));
                }
            }
        });
        ...
        for (var j = 0; j < classNameCount; j++) {
            ...
            if (cThemeMode === 'light' || themeMode === 'light') {
                ...
                aceEditor.setTheme('data-bs-theme', getCookie('site_auto_theme'));
            }
        }
    });
...
(function () {
    if (cThemeMode === 'light' || themeMode === 'light') {
        ...
        document.documentElement.setAttribute('data-bs-theme', getCookie('site_auto_theme'));
    }
})();
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res == 0) {
                ...
                $("#txtPrivateNoteTip").html("<i class='far fa-exclamation-triangle text-danger'></i> Invalid Tx Hash");
            }
        },
...
//function show(group, showAlt) {
//    group.querySelector('.default-content').style.display = showAlt ? 'none' : '';
//    group.querySelector('.alternate-content').style.display = showAlt ? '' : 'none';
//}

//function waitForCreative(container, group, n = 0) {
//    const hasCreative = container.querySelector('iframe,img') && container.offsetHeight > 0;
//    if (hasCreative) return show(group, false);
//    if (n < MAX_POLLS) {
//        return setTimeout(() => waitForCreative(container, group, n + 1), POLL_MS);
//    }
//    show(group, true);
//}

//window.addEventListener('DOMContentLoaded', () => {
//    document.querySelectorAll('[data-featured-group]').forEach(group => {
//        const container = group.querySelector('.featured-container');
//        if (!container) return;

//        /* Grab the first provider <script src> inside the container */
//        const tag = container.querySelector('script[src]');
//        if (!tag) {
//            return waitForCreative(container, group);
//        }

//        /* Probe the same URL with <link rel="preload"> (tiny request) */
//        const probe = document.createElement('link');
//        probe.rel = 'preload';
//        probe.as = 'script';
//        probe.href = tag.src;

//        probe.onerror = () => show(group, true);
//        probe.onload = () => waitForCreative(container, group);

//        document.head.appendChild(probe);
//    });
//});

function initTimestampSwitcher(elId, dropdownId, copyBtnId) {
    const el = document.getElementById(elId);
    const switcher = document.getElementById(dropdownId);
    const copyBtn = document.getElementById(copyBtnId);
    if (!el || !switcher) return;

    const modeInput = switcher.querySelector("#tsModeValue");
    const modeLabel = switcher.querySelector("#tsModeLabel");
    const triggerBtn = switcher.querySelector(".ts-dropdown-trigger");
    const tooltipTarget = switcher.querySelector(".ts-tooltip-target");
    const menuEl = switcher.querySelector(".dropdown-menu");
    const utcValueEl = switcher.querySelector("#tsUtcValue");
    const localValueEl = switcher.querySelector("#tsLocalValue");
    const unixValueEl = switcher.querySelector("#tsUnixValue");
    const copyButtons = switcher.querySelectorAll(".ts-copy-btn");

    const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    const timestamp = Number(el.dataset.timestamp);
    const date = new Date(timestamp * 1000);
    const tooltipTarget$ = tooltipTarget ? $(tooltipTarget) : null;

    if (switcher.dataset.tsInitialized === "1") {
        refreshMenuValues();
        const mode = getMode();
        setMode(mode);
        render(mode);
    ...
    function formatUTC(d) {
        ...
        return `${months[d.getUTCMonth()]}-${pad(d.getUTCDate())}-${d.getUTCFullYear()} ` +
            `${pad(h)}:${pad(d.getUTCMinutes())}:${pad(d.getUTCSeconds())} ${ampm} +UTC`;
    }
    ...
    function formatLocal(d) {
        ...
        const utcOffset = `UTC${sign}${pad(hours)}${minutes ? ":" + pad(minutes) : ""}`;
        return `${months[d.getMonth()]}-${pad(d.getDate())}-${d.getFullYear()} ` +
            `${pad(h)}:${pad(d.getMinutes())}:${pad(d.getSeconds())} ${ampm} ${utcOffset}`;
    }
    ...
    function getMode() {
        ...
        return modeInput.value.toLowerCase();
    }
    ...
    function refreshMenuValues() {
        if (utcValueEl) {
            utcValueEl.textContent = formatUTC(date);
        }
        if (localValueEl) {
            localValueEl.textContent = formatLocal(date);
        }
        if (unixValueEl) {
            unixValueEl.textContent = `${timestamp}`;
        }
    }
    ...
    function render(mode) {
        if (mode === "unix") {
            ...
            el.textContent = formatUTC(date);
        }
        if (modeLabel) {
            modeLabel.textContent = el.textContent;
        }
    }
    ...
    async function copyText(value, iconEl) {
        try {
            ...
            if (iconEl) {
                ...
                const button$ = $(iconEl);
                const originalTooltip = button$.attr("data-bs-original-title") || button$.attr("data-original-title") || button$.attr("title") || "Copy";
                const placement = button$.attr("data-bs-placement") || "top";
                const container = button$.attr("data-bs-container") || "body";
                ...
                setTimeout(() => {
                    ...
                    button$.tooltip("dispose");
                    button$.attr("title", originalTooltip);
                    button$.attr("data-bs-placement", placement);
                    button$.attr("data-bs-trigger", "hover");
                    button$.attr("data-bs-container", container);
                    renderFnTooltip(button$, 0);
                }, 800);
            }
        } catch (err) {
            console.error("Unable to copy timestamp value", err);
        }
    }
    ...
    copyButtons.forEach(icon => {
        icon.addEventListener("click", (event) => {
            ...
            let textToCopy = "";
            if (target === "local") {
                textToCopy = localValueEl ? localValueEl.textContent : "";
            } else if (target === "unix") {
                textToCopy = unixValueEl ? unixValueEl.textContent : "";
            } else {
                textToCopy = utcValueEl ? utcValueEl.textContent : "";
            }
            if (textToCopy) {
                copyText(textToCopy, icon);
            }
        });
    });
    ...
    if (tooltipTarget$ && tooltipTarget$.length) {
        tooltipTarget$.attr("data-bs-placement", "top");
        tooltipTarget$.attr("data-bs-trigger", "hover");
        tooltipTarget$.attr("data-bs-container", "body");
        renderFnTooltip(tooltipTarget$, 0);
    }
    ...
    if (triggerBtn && menuEl && !switcher.dataset.dropdownClickBound) {
        triggerBtn.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            toggleMenu();
        });
        switcher.dataset.dropdownClickBound = "1";
    }

    if (!switcher.dataset.outsideCloseBound) {
        document.addEventListener("click", (event) => {
            const isInsideSwitcher = switcher.contains(event.target);
            if (!isInsideSwitcher) {
                hideMenu();
            }
        }, true);
        switcher.dataset.outsideCloseBound = "1";
    }
    ...
    if (copyButtons && copyButtons.length) {
        copyButtons.forEach((btn) => {
            ...
            button$.attr("data-bs-trigger", "hover");
            button$.attr("data-bs-container", "body");
            renderFnTooltip(button$, 0);
        });
    }

    switcher.dataset.tsInitialized = "1";
