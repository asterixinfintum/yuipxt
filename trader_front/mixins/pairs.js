export default {
    ...
    methods: {
        initialrender() {
            const { current, pairslist } = this;

            const pr = pairslist.filter(pair => {
                return pair.pairlabel.includes(current);
            });

            return this.currentpr = pr[0];
        },
        togglepair(pair) {
            const { pairslist } = this;

            const pr = pairslist.filter(pairitem => {
                return pairitem.pair === pair.pair;
            });
    ...
    computed: {
        includedpairs() {
            ...
            this.pairslist.forEach(item => {
                includedpairs.push(item.pair)
            });
